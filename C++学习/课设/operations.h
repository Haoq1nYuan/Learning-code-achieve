...
void Add_Media()
{
    ...
    while (true)
    {
        ...
        if (oper.size() != 1) std::cout << "[                请输入合法的类别索引！                ]" << std::endl;
        else if (stoi(oper) > 3 || stoi(oper) < 1) std::cout << "[                请输入合法的类别索引！                ]" << std::endl;
        else break;
    }
...
void Update_Media()
{
    if (ID_list.size())
    {
        ...
        if (!ID_list[stoi(Id)]) std::cout << "[                 不存在该编号的媒体！                 ]" << std::endl << std::endl;
        else 
        {
...
void Search_Media()
{
    if (ID_list.size())
    {
        while (true)
        {
            ...
            if(oper.size() != 1) std::cout << std::endl << "[                请输入合法的类别索引！                ]" << std::endl << std::endl;
            else if (stoi(oper) > 3 || stoi(oper) < 1) std::cout << std::endl << "[                请输入合法的类别索引！                ]" << std::endl << std::endl;
            else break;
        }
        ...
        if (type == 1)
        {
            ...
            if (!Name_to_ID[Name].size()) std::cout << std::endl << "[                   该标题不存在！                     ]" << std::endl;
            else
            {
                std::cout << std::endl << "找到了！媒体信息如下：" << std::endl << std::endl;
        ...
        else if (type == 2)
        {
            ...
            if (!ID_list[stoi(Id)]) std::cout << std::endl << "[                    该编号不存在！                    ]" << std::endl;
            else
            {
                std::cout << std::endl << "找到了！媒体信息如下：" << std::endl << std::endl;
                Type = ID_list[stoi(Id)];
        ...
        else
        {
            ...
            if (Type == 2) 
            {
                if (!Book_id_list.size()) std::cout << "[                   书籍类没有物品！                   ]" << std::endl;
                else Search_Media_in_Type(Book_id_list);
            }
            if (Type == 3) 
            {
                if (!Drawing_id_list.size()) std::cout << "[                   图画类没有物品！                   ]" << std::endl;
...
void Delete_Media()
{
    if (ID_list.size())
    {
        while (true)
        {
            ...
            if (!ID_list[stoi(Id)]) std::cout << std::endl << "[                    该编号不存在！                    ]" << std::endl << std::endl;
            else break;
        }
...
void writeMapToFile(Media m, const std::string& filename, int type)
{
    ...
    if (outFile.is_open())
	{
        ...
        outFile.close();
        std::cout << "[                      保存成功！                      ]" << std::endl << std::endl;
    } 
