...
int pre_path_g1[szx + 1][szy + 1][2];
int pre_path_g2[szx + 1][szy + 1][2];
        ...
        for (int k = 0; k < 4; k++)
        {
            ...
            if (nx >= 1 && nx <= szx && ny >= 1 && ny <= szy && !used[link[nx][ny]])
                ...
                {   
                    ...
                    pre_path_g1[nx][ny][0] = tem.first, pre_path_g1[nx][ny][1] = tem.second; 
                    kk.push({nx, ny});
                }
                else if (mode == 3 && a[nx][ny] != 4 && dist_g2[nx][ny] == -1)
                {
                    dist_g2[nx][ny] = dist_g2[tem.first][tem.second] + 1;
                    pre_path_g2[nx][ny][0] = tem.first, pre_path_g2[nx][ny][1] = tem.second;
                    kk.push({nx, ny});
                }
        }
    }
...
char solve()
{
    ...
    */   
    ...
    if (preghost2.second.first && min(abs(person_x - preghost2.first.first) + abs(person_y - preghost2.first.second), abs(person_x - preghost2.second.first) + abs(person_y - preghost2.second.second)) <= 1) 
    {
        flag = true;
    ...
    priority_queue<dist> final_dist;

    for (int i = 1; i < idx_been; i++) 
    {
        dist d;
        d.dist_p = dist_p[link_back[i][0]][link_back[i][1]];
        d.dist_g = dist_g1[link_back[i][0]][link_back[i][1]];
        if (preghost2.second.first) d.dist_g = min(d.dist_g, dist_g2[link_back[i][0]][link_back[i][1]]);
    ...
    while (final_dist.size())
    {
        ...
        if (flag && (ans_dist.dist_p < ans_dist.dist_g || (ans_dist.dist_p == ans_dist.dist_g && ans_dist.dist_p > 1))) break;
        if (!flag) break;
    }

    int st_x = ans_dist.tarx, st_y = ans_dist.tary;
    ...
    if (st_x == ans_dist.tarx && st_y == ans_dist.tary) used[link[st_x][st_y]] = true;
 
    if (move_x == -1 && move_y == 0) last_move = 'U';
    else if (move_x == 0 && move_y == -1) last_move = 'L';
    else if (move_x == 1 && move_y == 0) last_move = 'D';
    else last_move = 'R';
...
// 99，优化不同地图之间存储结构的刷新，规定只当鬼与人相距为1时才考虑鬼的行为，减少不必要的bfs调用